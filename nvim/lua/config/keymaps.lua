...
-- ====================================================
-- Surround selected text (plugin-free)
-- ====================================================
...
local function surround_selection(left, right)
  ...
  if #lines == 1 then
    lines[1] = string.sub(lines[1], 1, scol - 1)
      .. left
      .. string.sub(lines[1], scol, ecol)
      .. right
      .. string.sub(lines[1], ecol + 1)
    vim.fn.setline(srow, lines[1])
  else
    lines[1] = string.sub(lines[1], 1, scol - 1) .. left .. string.sub(lines[1], scol)
    lines[#lines] = string.sub(lines[#lines], 1, ecol) .. right .. string.sub(lines[#lines], ecol + 1)
    vim.fn.setline(srow, lines)
  end
end

local surround_opts = { noremap = true, silent = true }
